...
if (N<0 or freq<0 or rad<0 or eps<0 or tmax<0 or step<0):
	print 'Error: Parameters not passed properly to wrapper script'
	sys.exit(1)
	
instance_specific_string = ','.join(map(str,inst_specifics))
seq = (instance_name, instance_specific_string, str(cutoff_time), str(cutoff_length), str(seed), str(N), str(rad), str(freq), str(eps), str(step), str(tmax))	

paramstring = ','.join(seq)
	
# Create command and output filename
filename = './paramils-output/mpso_run0_output_'+time.strftime("%Y%m%d-%H%M%S")+'.txt'
...
res = eng.MPSO(instance_name,xmin,xmax,true_min,errgoal,filename,cutoff_time,cutoff_length,seed,N,rad,freq,eps,step,tmax,nargout=3)

if not(os.path.isfile(filename)):
	print 'Error: MATLAB did not create output file'
	print 'Result for ParamILS: CRASHED, '+str(cutoff_time)+', '+str(cutoff_length)+', -1, '+str(seed)
